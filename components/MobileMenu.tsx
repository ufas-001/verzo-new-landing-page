...
import { ArrowUpDown, Mail, Phone, PlusCircle, Scroll, X } from "lucide-react";
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@/components/ui/sheet";
import Verzologo from "@/app/components/Verzologo";
...
import AccordionDownIcon from "@/app/components/AccordionDownIcon";
...
const Mobilemenu: React.FC<MobileMenuProps> = ({ open, onClose }) => {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent
        className="md:rounded-b-[30px] rounded-b-[20px] bg-primary-background"
        side={"top"}
      >
        <div className="mx-auto w-[90%] h-[600px] flex flex-col py-[30px]">
          <SheetHeader>
            <SheetTitle className="text-2xl font-semibold text-primary-black">
            </SheetTitle>
            <div className="flex flex-row justify-between items-start h-[60px]">
              ...
              <X
                className="text-primary-black w-6 h-6 mt-2 cursor-pointer"
                onClick={() => onClose()}
              />
            </div>
          </SheetHeader>
          <div className="flex-grow flex flex-col justify-between">
            <div className=" h-[400px] overflow-y-scroll flex">
              <Accordion
                type="single"
                collapsible
                className="w-full text-lg text-primary-greytext"
              >
                <Link href="/about">
                  <button className=" hover:text-gray-800 py-3.5 font-normal flex items-center text-primary-greytext cursor-pointer">
                    About us
                  </button>
                </Link>
                <AccordionItem className=" py-3.5" value="item-2">
                  <AccordionTrigger className=" flex data-[state=open]:text-gray-800 hover:text-gray-800 items-center font-normal [&[data-state=open]>svg]:rotate-180 [&[data-state=closed]>svg]:text-gray-300">
                    Features
                    <AccordionDownIcon />
                  </AccordionTrigger>
                  <AccordionContent>
                    <div className="flex flex-row flex-wrap gap-x-3 gap-y-5 justify-between pt-4">
                      <div className="flex flex-col max-w-[230px] gap-y-8">
                        <div className="flex-1 flex flex-col gap-y-4">
                          <h2>By Product</h2>
                          <div className="flex flex-col gap-y-4">
                            {features[0].byProduct.map((product, index) => (
                        ...
                        <div className="flex-1">
                          <div className="flex flex-col gap-y-4 mb-2">
                            <h2>By Use case</h2>
                            {features[0].byUseCase.map((useCase, index) => (
                              <div key={index}>
                                <p>{useCase.header}</p>
                              </div>
                            ))}
                      ...
                      <button className="w-full max-w-[250px] md:max-w-[400px] md:h-[220px] md:p-[32px] h-[190px] bg-gray-100 cursor-default rounded-[24px] flex flex-col justify-center items-start gap-y-[24px] p-[18px]">
                        <p className="text-sm text-primary-greytext">
                          Useful links
                        </p>
                        <ul className="flex flex-col gap-y-[20px]">
                          <Link href="https://beta.verzo.app/" target="_blank">
                            <li className="flex flex-row gap-x-[14px] items-center cursor-pointer text-[15px] font-medium text-gray-600">
                          ...
                          <ul>
                            <Link
                              target="_blank"
                              href="https://blog.verzo.app/"
                            >
                              <li className="flex flex-row gap-x-[14px] items-center text-[15px] font-medium text-gray-600">
                ...
                <AccordionItem className=" py-3.5" value="item-3">
                  <AccordionTrigger
                    disabled
                    className=" flex disabled:opacity-50 disabled:cursor-not-allowed data-[state=open]:text-gray-800 hover:text-gray-800 items-center font-normal [&[data-state=open]>svg]:rotate-180 [&[data-state=closed]>svg]:text-gray-300"
                  >
                    Resources
                    <AccordionDownIcon />
                  </AccordionTrigger>
                  <AccordionContent>
                    <div className="flex flex-row flex-wrap gap-x-3 gap-y-5 justify-between pt-4">
                      <div className="flex flex-col max-w-[230px] gap-y-8 pt-2">
                        <div className="flex-1 flex flex-col gap-y-4">
                          <div className="flex flex-col gap-y-4">
                            {resources[0].byProduct.map((product, index) => (
                              <Link key={index} href={product.link}>
                                <button className="flex items-center gap-x-3">
                                  <h3>{product.header}</h3>
                                </button>
            ...
            <div className=" flex justify-between items-center">
              <Link target="_blank" href="https://beta.verzo.app/">
                <button className="text-lg font-normal leading-6 text-primary-black">
                  Log in
                </button>
              </Link>
              <Link href="https://beta.verzo.app/" target="_blank">
                <button className="rounded-[8px] bg-primary-blue px-6 py-2 text-base text-white">
                  Sign up
                </button>
              </Link>
            </div>
